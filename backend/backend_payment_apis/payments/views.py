    ...
    def post(self, request):
        try:
            ...
            if response.status_code == 200:
                ...
                firestore_client.collection("mytransactions").document(
                    transaction_id
                ).set(
                    {
                        "id": transaction_id,
                        "amount": str(
                            amount
                        ),  # Firestore does not support Decimal, so convert to string
                        "phone_number": phone_number,
...
class BalanceView(APIView):
    def get(self, request):
        try:
            ...
            docs = transactions_ref.where(
                field_path="userId", op_string="==", value=user_id
            ).stream()
            ...
            pendingBalance = sum(
                ...
                for transaction in transactions
                if transaction.get("status") == "COMPLETE"
                and transaction.get("metadata", {}).get("type") == "pending"
            )
...
class CardDepositView(APIView):
    def post(self, request):
        ...
        response = requests.post(
            "https://sandbox.intasend.com/api/v1/checkout/", json=intasend_data
        )
...
class CheckoutStatus(APIView):
    def post(self, request):
        ...
        # Send request to IntaSend API
        response = requests.post(
            "https://sandbox.intasend.com/api/v1/checkout/details/", json=intasend_data
        )
        if response.status_code == 200:
            data = response.json()
            return Response(data, status=response.status_code)
...
class TransactionDetailView(APIView):
    def get(self, request, transaction_id):
        try:
            ...
            transaction_ref = firestore_client.collection("mytransactions").document(
                transaction_id
            )
...
class SavePinView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                db.collection("users_store_pin").document(str(user_id)).set(
                    {"pin": pin}, merge=True
                )
                return Response(
                    {"message": "PIN saved successfully"}, status=status.HTTP_200_OK
                )
            except Exception as e:
                return Response(
                    {"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
