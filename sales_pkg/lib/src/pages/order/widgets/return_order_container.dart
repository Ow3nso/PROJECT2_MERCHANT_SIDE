...
class ReturnedOrderContainer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: FutureBuilder(
          future: context.read<OrderController>().getReturnedOrders(),
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              if (context.watch<OrderController>().returnedOrders.isEmpty) {
                return DefaultMessage(
                  ...
                  label: 'Refresh',
                  onTap: () {
                    context.read<OrderController>().getReturnedOrders();
                  },
                );
              }
              return RefreshIndicator(
                child: ListView.builder(
                  itemCount:
                      context.watch<OrderController>().returnedOrders.keys.length,
                  itemBuilder: (BuildContext context, int index) {
                    var order = context
                        .read<OrderController>()
                        .order(index, context.watch<OrderController>().returnedOrders);
