...
class DraftController extends ChangeNotifier {
  ...
  Map<String, Product> _drafts = {};

  /// > This is the products map
  Map<String, Product> get drafts => _drafts;
  set drafts(Map<String, Product> drafts) {
    _drafts = drafts;
    notifyListeners();
  }
  ...
  Future<bool> getDraftProducts({bool isrefreshMode = false}) async {
    ...
    try {
      var productsDocs = await db
          ...
          .where(ProductFields.isApproved, isEqualTo: true)
  ...
  Future<bool> getProducts({bool isrefreshMode = false}) async {
    ...
    try {
      var productsDocs = await db
          ...
          .where(ProductFields.sellerId,
              isEqualTo: context.read<UserRepository>().fbUser!.uid)
          .where(ProductFields.isApproved, isEqualTo: false)
          // .orderBy(ProductFields.createdAt, descending: true)
          .get();
      if (productsDocs.docs.isNotEmpty) {
        products = {
