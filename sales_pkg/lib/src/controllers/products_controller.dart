...
class ProductController extends ChangeNotifier {
  ...
  // List<Product> get drafts => _drafts;

  Map<String, Product> _draftProducts = {};

  /// > This is the products map
  Map<String, Product> get draftProducts=> _draftProducts;
  set draftProducts(Map<String, Product> draftProducts) {
    _draftProducts = draftProducts;
    notifyListeners();
  }
  ...
  //     _drafts = snapshot.docs.map((doc) => Product.fromJson(doc.data() as Map<String, dynamic>)).toList();
  //     notifyListeners();
  //   } catch (e) {
  //     if (kDebugMode) {
  ...
  /// 
  Future<bool> getDraftProducts({bool isrefreshMode = false}) async {
    if (draftProducts.isNotEmpty && !isrefreshMode) return true;
  ...
  Future<bool> getProducts({bool isrefreshMode = false}) async {
    ...
    try {
      var productsDocs = await db
          ...
          .where(ProductFields.sellerId,
              isEqualTo: context.read<UserRepository>().fbUser!.uid)
          .where(ProductFields.isAvailable, isEqualTo: true)
          // .orderBy(ProductFields.createdAt, descending: true)
          .get();
      if (productsDocs.docs.isNotEmpty) {
        products = {
