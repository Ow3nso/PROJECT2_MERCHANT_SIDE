...
class UploadProductController extends ChangeNotifier {
  ...
  /// Whether the product is available (true) or a draft (false)
  ...
  //   try {
  //     // Save the product details to the drafts collection in your database
  //     // Example:
  ...
  //       'images': pickedItemFiles,
  //     });

  //     // For demonstration purposes, we'll just log the details
  //     log('Product saved to drafts: ${productNameController.text}');
  //     return true;
  //   } catch (e) {
  //     log('Error saving to drafts: $e');
  //     return false;
  //   }
  // }

  Future<bool> initDraftProduct({Product? originalProduct}) async {
    ...
    label: productNameController.text.trim().isNotEmpty
        ? productNameController.text.trim()
        : "",
    description: productDescriptionController.text.trim().isNotEmpty
        ? productDescriptionController.text.trim()
        : "",
    price: double.tryParse(productPriceController.text.trim()) ?? 0.0, // Default price
    availableSizes:
        isVailableInVariousSizes ? ['Various sizes'] : (sizes.isNotEmpty ? sizes : ['N/A']),
    stock: int.tryParse(productQuantityController.text.trim()) ?? 0, // Default stock
    isApproved: true, // Keeping this for now, but could be false for drafts
    category: category ?? "",
    subCategory: condition ?? "",
    mediaPath: mediaPath,
    shopId: userRepo.shop?.shopId ?? "",
    availableColors: selectedColors.isNotEmpty ? [...selectedColors] : [""],
  );
  ...
  /// Returns:
  ///   A Future<bool>
  Future<bool> saveDraftProduct() async {
  ...
  Future<bool> initProduct({Product? originalProduct}) async {
    ...
    // For drafts (isAvailable = false), we only need to check if there's at least one image
    if (!isAvailable) {
      if (pickedItemFiles.where((path) => path != null).isEmpty) {
        uploading = false;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Please add at least one image or video for draft')));
        return false;
      }
    } else {
      // For non-drafts (isAvailable = true), validate all required fields
      if (productNameController.text.trim().isEmpty ||
          productDescriptionController.text.trim().isEmpty ||
          productPriceController.text.trim().isEmpty ||
          productQuantityController.text.trim().isEmpty) {
        uploading = false;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
    ...
    product = product!.copyWith(
        ...
        isAvailable: isAvailable); // Include the isAvailable field
  ...
  bool isProductDataValid({required Map<String, dynamic> data}) {
    // Check if this is a draft (isAvailable = false)
    final isDraft = data['isAvailable'] == false;
    
    if (isDraft) {
      // For drafts, only validate these essential fields
      ...
      for (final field in draftRequiredFields) {
        if (data[field] == null || data[field] == '') {
          return false;
        }
      }
    } else {
      // For non-drafts (regular products), validate all required fields
